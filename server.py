...
@MetricsHandler.track_subprocess_exit_code
def create_ffmpeg_stream(video_path:str, video_type:State, loop=False):
    ...
    if video_type == State.PLAYING:
        MetricsHandler.streams_count.labels("playing").inc(amount=1)
    else:
        MetricsHandler.streams_count.labels("interlude").inc(amount=1)
